...
describe("claude restart recovery (issue #22)", () => {
  ...
  it("recovers a finished claude task via its persisted claudeSessionId, not sessionName", () => {
    ...
    const closes: string[] = [];
    const backgroundTasks = new Map();
    restoreActiveBackgroundTasks(piDir, backgroundTasks, () => false, (entry) => {
  ...
  it("restores a running claude task with the UUID-rebuilt transcript path", () => {
    const piDir = makePiDir();
    const cwd = join(piDir, "repo");
  ...
  it("never invents a claude transcript path from a non-UUID sessionName", () => {
  ...
  it("falls back to a legacy UUID sessionName when claudeSessionId is absent", () => {
